    ...
    <header className="sticky top-0 z-50 flex h-14 w-full items-center justify-between border-b border-border-color bg-header-bg px-4 shadow-sm">
      ...
      <div className="flex items-center">
        <Link href="/" className="mr-2 flex items-center">
          <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-accent-blue via-purple-600 to-pink-500 text-white shadow-lg transition-transform hover:scale-105">
        ...
        <div className="relative">
          <form onSubmit={handleSearch}>
            <div
              className={`flex items-center rounded-full bg-card-bg transition-all duration-300 ease-in-out ${
                searchActive ? 'w-72 shadow-md ring-2 ring-accent-blue/30' : 'w-44'
              }`}>
              <div className="flex h-10 w-10 items-center justify-center text-text-secondary">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={`h-5 w-5 transition-all duration-300 ${searchActive ? 'text-accent-blue' : ''}`}>
              ...
              {searchQuery && (
                <button
                  type="button"
                  className="mr-2 rounded-full p-1 text-text-secondary hover:bg-hover-bg hover:text-text-primary"
          ...
          {/* ძიების ისტორია */}
          {showSearchHistory && (
            <div className="absolute left-0 top-12 w-72 rounded-lg border border-border-color bg-card-bg p-2 shadow-lg">
              <div className="mb-2 flex items-center justify-between border-b border-border-color pb-1">
                <h4 className="text-sm font-medium text-text-primary">Recent Searches</h4>
                <button className="text-xs text-accent-blue hover:underline">Clear All</button>
              </div>
              <ul className="space-y-1">
                {recentSearches.map((search, index) => (
                  <li key={index}>
                    <button
                      className="flex w-full items-center rounded-md px-2 py-1.5 text-left text-text-primary hover:bg-hover-bg"
                      onClick={() => {
                        setSearchQuery(search);
                        setShowSearchHistory(false);
                      }}
                    >
                      <div className="mr-2 flex h-8 w-8 items-center justify-center rounded-full bg-card-secondary-bg">
      ...
      <div className="flex items-center gap-2">
        ...
        {/* Messenger */}
        <button className="relative flex h-10 w-10 items-center justify-center rounded-full bg-card-secondary-bg hover:bg-hover-bg transition-transform active:scale-95">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-6 w-6">
          ...
          <span className="absolute -right-1 -top-1 flex min-w-[18px] h-[18px] items-center justify-center rounded-full bg-accent-red px-[2px] text-xs font-bold text-white animate-[pulse_1.5s_ease-in-out_infinite] shadow-sm">3</span>
          <span className="absolute inset-0 rounded-full bg-accent-blue/20 animate-ping opacity-75"></span>
        </button>
        ...
        <button className="relative flex h-10 w-10 items-center justify-center rounded-full bg-card-secondary-bg hover:bg-hover-bg transition-transform active:scale-95">
          ...
          <span className="absolute -right-1 -top-1 flex min-w-[18px] h-[18px] items-center justify-center rounded-full bg-accent-red px-[2px] text-xs font-bold text-white animate-[pulse_1.5s_ease-in-out_infinite] shadow-sm">5</span>
          <span className="absolute inset-0 rounded-full bg-accent-red/20 animate-ping opacity-75"></span>
        </button>

        {/* Profile */}
        <div className="relative">
          <button className="flex h-10 w-10 items-center justify-center overflow-hidden rounded-full ring-2 ring-accent-blue transition-all hover:ring-opacity-100 ring-opacity-0">
            <Image
